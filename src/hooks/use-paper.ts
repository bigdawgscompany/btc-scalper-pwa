...
export function usePaper(
  ...
  const runEvaluation = useCallback(async () => {
    ...
    try {
      ...
      const persistedAccount = await recordEvaluationStep(
        updatedAccount,
        decision,
        fills,
        closedTrade,
      ...
      accountRef.current = persistedAccount;
  ...
  // Mark-to-market is updated from executable bid/ask quotes inside the paper engine.
  // Do not overwrite a live position with a candle-close mark, because that would
  // create a second pricing model distinct from the simulated execution venue.
