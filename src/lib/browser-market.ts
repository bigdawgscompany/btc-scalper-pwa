...
export function validateContributionArithmetic(
  ...
  if (contributions.length !== 4 && analysis.state !== "Unavailable") {
    errors.push(`Expected 4 group contributions, got ${contributions.length}`);
  }
  ...
  for (const g of contributions) {
    ...
    const expectedContribution = g.vote * 25;
  ...
  const expectedScore = 25 * Math.abs(sumVotes);
...
/**
 * Validates score/direction consistency: Bullish requires sumVotes >= 3,
 * Bearish requires sumVotes <= -3, Wait otherwise.
 */
...
export function isEligibleForExecution(
  ...
  if (
    data.confirmedAnalysis.state === "Unavailable" ||
    data.confirmedAnalysis.state === "Wait"
  ) {
    return { eligible: false, reason: `Signal state: ${data.confirmedAnalysis.state}` };
  }
