...
export function validateAccountInvariants(
  ...
  if (account.position) {
    ...
    if (!account.position.entryFillId) {
      errors.push("Position missing entryFillId");
    }
  }
...
export async function getOrInitAccount(
  ...
  if (existing) {
    if (existing.position && !existing.position.entryFillId) {
      const entrySide = existing.position.side === "LONG" ? "BUY" : "SELL";
      const candidate = await db.fills
        .where("side")
        .equals(entrySide)
        .filter(
          (fill) =>
            fill.quantity === existing.position!.quantity &&
            fill.timestamp <= existing.position!.entryTime &&
            existing.position!.entryTime - fill.timestamp <= 5000
        )
        .last();
      if (candidate) {
        existing.position = { ...existing.position, entryFillId: candidate.id };
        await db.account.put(existing);
      }
    }
...
export async function saveAccount(account: PaperAccountState): Promise<void> {
  const db = getDatabase();
  await db.transaction("rw", db.account, async () => {
    const current = await db.account.get(account.id);
    if (current && current.revision !== account.revision) {
      throw new Error(
        `Account revision conflict: expected ${account.revision}, found ${current.revision}.`
      );
    }
    const invariantErrors = validateAccountInvariants(account);
    if (invariantErrors.length > 0) {
      throw new Error(`Refusing to save invalid account: ${invariantErrors.join("; ")}`);
    }
...
export async function recordEvaluationStep(
  ...
  const persistedAccount = await db.transaction(
    "rw",
    [
      db.account,
      db.decisions,
    ...
    async () => {
      // 1. Compare-and-swap account revision before recording any journal rows.
      const current = await db.account.get(account.id);
      if (!current) {
        throw new Error(`Cannot record evaluation: account ${account.id} does not exist.`);
      }
      ...
      const invariantErrors = validateAccountInvariants(account);
      if (invariantErrors.length > 0) {
        throw new Error(`Refusing to record invalid account: ${invariantErrors.join("; ")}`);
      }

      const persistedAccount: PaperAccountState = {
        ...account,
        revision: account.revision + 1,
        updatedAt: Date.now(),
      };
      await db.account.put(persistedAccount);
...
/**
 * Validate backup child records and their cross-record references before import.
 */
function validateBackupRecords(
  ...
  for (const fill of fills) {
    if (!fill.id || !decisionIds.has(fill.decisionId)) {
      errors.push(`Fill ${fill.id || "<missing>"} references missing decision ${fill.decisionId || "<missing>"}`);
    }
  }

  for (const trade of trades) {
    if (!fillIds.has(trade.entryFillId)) {
      errors.push(`Trade ${trade.id} references missing entry fill ${trade.entryFillId}`);
    }
    if (!fillIds.has(trade.exitFillId)) {
      errors.push(`Trade ${trade.id} references missing exit fill ${trade.exitFillId}`);
    }
  }

  for (const snapshot of snapshots) {
    if (!snapshot.id || !Number.isFinite(snapshot.timestamp) || !Number.isFinite(snapshot.candleTime) || !Number.isFinite(snapshot.score)) {
      errors.push(`Signal snapshot ${snapshot.id || "<missing>"} contains invalid scalar data`);
    }
    if (!Array.isArray(snapshot.reasons)) {
      errors.push(`Signal snapshot ${snapshot.id || "<missing>"} has invalid reasons`);
    }
  }
...
export async function importBackupJSON(jsonString: string): Promise<PaperAccountState> {
  ...
  const equityObservations = Array.isArray(parsed.equityObservations)
    ? parsed.equityObservations
    : [];
  const snapshots = Array.isArray(parsed.snapshots) ? parsed.snapshots : [];
  errors.push(...validateBackupRecords(trades, fills, decisions, snapshots));
  if (errors.length > 0) {
  ...
  await db.transaction(
    ...
    async () => {
      ...
      if (fills.length > 0) await db.fills.bulkPut(fills);
      if (decisions.length > 0) await db.decisions.bulkPut(decisions);
      if (equityObservations.length > 0) {
        await db.equityObservations.bulkPut(equityObservations);
      }
      if (snapshots.length > 0) await db.signalSnapshots.bulkPut(snapshots);
    }
  );
