...
/**
 * Create a fresh paper-trading account using the supplied configuration.
 */
...
/**
 * Calculate a fee from notional value and fee basis points.
 */
...
export function evaluateAutopilotCycle(
  ...
  // 1. Existing position: Evaluate Stops & Targets first — ALWAYS runs, on
  // every tick, regardless of the signal-decision dedup below. Stop/target
  // depend on live bid/ask, not on the confirmed candle's signal state, so
  // gating this behind a once-per-candle dedup (as the previous version did)
  // meant a position's stop-loss/take-profit only got checked on the first
  // evaluation of each ~15-minute candle and was silently unmonitored for
  // the rest of the window — a real risk-management gap, not a cosmetic one.
  ...
  // Decision dedup: skip re-running the SIGNAL-driven paths (reversal/entry,
  // below) if this confirmed candle was already evaluated with the same
  // strategy version. The confirmed signal only changes once per candle, so
  // this is safe here — unlike the stop/target check above, which just ran
  // unconditionally and is never gated by this. Applies whether flat or in
  // a position, so a flat account doesn't spam repeated entry attempts every
  // 2s tick either.
  const alreadyEvaluatedThisCandle =
    currentAccount.lastCandleEvaluated === candleTime &&
    currentAccount.lastStrategyVersionEvaluated === currentAccount.config.strategyVersion &&
    currentAccount.lastExitCandleTime !== candleTime;

  if (alreadyEvaluatedThisCandle) {
    const decision: PaperDecision = {
      id: decisionId,
      timestamp: Date.now(),
      confirmedCandleTime: candleTime,
      strategyVersion: currentAccount.config.strategyVersion,
      action: "HOLD",
      reason: `Candle ${candleTime} signal already evaluated — deduplicated (stop/target still monitored live)`,
      signalState: analysis.state,
      signalScore: analysis.score,
      quoteBid: quote.bid.toString(),
      quoteAsk: quote.ask.toString(),
  ...
  if (currentAccount.position) {
    ...
    if (currentAccount.config.allowOppositeReversal && (isOppositeBullish || isOppositeBearish)) {
      ...
      try {
        ...
        // The old position is already closed; failed reopening leaves the
        // account flat and must be recorded truthfully in the decision.
      }
      ...
      const decision: PaperDecision = {
        ...
        confirmedCandleTime: candleTime,
        strategyVersion: currentAccount.config.strategyVersion,
        action: reversed ? "REVERSE" : "EXIT_OPPOSITE",
        reason: reversed
          ? `Opposite confirmed ${analysis.state} signal triggered reversal`
          : `Opposite confirmed ${analysis.state} signal closed position; replacement entry failed`,
        signalState: analysis.state,
        signalScore: analysis.score,
        quoteBid: quote.bid.toString(),
        quoteAsk: quote.ask.toString(),
